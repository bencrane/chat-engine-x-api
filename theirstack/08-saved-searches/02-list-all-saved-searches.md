# TheirStack - Saved Searches - List All Saved Searches
...
**Endpoint:** `GET /v0/saved_searches`
...
|---|---|---|---|
| `user_ids` | `array<integer>` | `[]` | Get saved searches from users with these IDs. The IDs must be from people in the current users' team. |
| `types` | `array<string>` (nullable) | — | Filter by type of search. Values: `companies`, `jobs` |
| `order_by` | `string` | `"updated_at"` | Order by field. Values: `name`, `created_at`, `updated_at` |
| `order_direction` | `string` | `"desc"` | Order direction. Values: `asc`, `desc` |
...
```bash
curl -X GET "https://api.theirstack.com/v0/saved_searches" \
