# Company Enrichment - Batch Enrich
...
Each domain is mapped to a unique company, making domain lookups fast and reliable. Batching is recommended for larger lists — it eliminates multiple requests and handles rate limiting more efficiently.
...
|-----------|----------|----------|-------------|
| `domains` | string[] | Yes      | List of domains to enrich. Length between 1 and 50. |
...
Returns an **array of objects**. Each object has the same schema as the single enrich endpoint.

### Top-Level Fields (per object)
...
|---------------|----------------|-------------|
| `country`     | object \| null | Country info (`code`, `name`, `latitude`, `longitude`) |
| `state`       | object \| null | State info (`id`, `name`, `code`, `latitude`, `longitude`) |
| `city`        | object \| null | City info (`id`, `name`, `latitude`, `longitude`) |
