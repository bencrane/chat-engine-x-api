...
Firstly, make sure that Python 3 is installed correctly. You can also try to substitute `pip3` with `pip` and execute the install command.

If that doesn’t work, Python 3 is likely accessible from a local directory.
  ...
  5. If `echo $PATH` does not give you the path, then you can find out where Profiles Builder is installed using `pip3 show profiles-rudderstack`. This command displays a list of the files associated with the application, including the location in which it was installed. You can navigate to that directory.
  6. Navigate to the `/bin` subdirectory and execute the `ls` command to confirm that `pb` is present there.
  7. To add the path of the location where PB is installed via pip3, execute: `export PATH=$PATH:<path_to_application>`. This will add the path to your system’s PATH variable, making it accessible from any directory. Note that the path should be complete and not relative to the current working directory.


If you still face issues, you can try to install it manually. Contact [RudderStack Support](<mailto:support@rudderstack.com>) for the executable file and download it on your machine. Follow these steps afterwards:
  ...
  4. Navigate to the directory `/usr/local/rudderstack` from your file explorer. Ctrl+Click on pb and select **Open** to run it from Terminal.
  5. Symlink to a filename pb in `/usr/local/bin` so that the command can locate it from the env PATH. Create a file if it does not exist: `sudo touch /usr/local/bin/pb`. Then execute `sudo ln -sf /usr/local/rudderstack/pb /usr/local/bin/pb`.
  6. Verify the installation by running `pb` in Terminal. In case you get the error `command not found: pb`, then check if `/usr/local/bin` is defined in PATH by executing the command: `echo $PATH`. If not, then add `/usr/local/bin` to PATH.


  1. If the Windows firewall prompts you after downloading, proceed with `Run Anyway`.
  2. Rename the executable as `pb`.
  3. Move the file to a directory, for example, `C:\Program Files\Rudderstack`. You can create the directory if required.
  4. Set the path of the `pb.exe` file in the environment variables.
  5. Verify the installation by running `pb` in the command prompt.
...
Profiles support git branches in the RudderStack dashboard. Refer [Supported Git URLs](<https://www.rudderstack.com/docs/profiles/additional-resources/git-url/>) for more information.
...
**What is the difference between the`valid_at` and `first_seen_at` columns in the ID graph?**

In the context of the ID Stitcher output, the columns `valid_at` and `first_seen_at` capture timestamps for each record related to the edges and clusters created during the connected component analysis.

  * `first_seen_at`: The earliest timestamp at which a specific identifier (node) was first observed in the source data. It is set once when the node first appears and does not change when the cluster is later modified or expanded. Each node retains its own independent `first_seen_at` value.
  * `valid_at`: Reflects the validity of this node’s current cluster assignment. It is initially derived from the minimum timestamp of the node’s source edges. However, during the stitching process, when clusters merge, `valid_at` is recalculated for affected nodes. This means `valid_at` may update for existing nodes in a cluster when new identifiers join, even if those specific nodes had no new activity.


Consider a situation where Node A and Node B are first linked on `2024-01-01`. Both `valid_at` and `first_seen_at` for this cluster would be `2024-01-01`.

Later, Node C connects to this cluster on `2024-02-15`:

  * Node C’s `first_seen_at` = `2024-02-15` (when Node C was first observed in the source data).
  * Node C’s `valid_at` = `2024-02-15` (when its cluster connection was established).
  * Node A and B’s `first_seen_at` remains `2024-01-01` (unchanged).
  * Node A and B’s `valid_at` may update to `2024-02-15` as a result of the cluster merge during stitching.


In summary:

  * `first_seen_at` is a stable, per-node timestamp recording when each identifier was first observed. Use this field when you need a reliable timestamp for individual identifiers (for example, to determine a primary email address).
  * `valid_at` reflects the cluster assignment validity and can change when the cluster is modified. It should not be used as a stable per-node timestamp.
...
**How can I fix the issue of identity explosion, that is, too many connections between identities?**

You can use [ID Graph Cardinality Rules](<https://www.rudderstack.com/docs/profiles/dev-docs/advanced-id-stitcher-features/id-graph-cardinality-rules/>) to limit connections between ID types.
...
Yes, refer to the [YAML best practices](<https://www.rudderstack.com/docs/profiles/dev-docs/yaml-refresher/>) guide for detailed information.
...
You can use an escape character. For example:
      ...
      - entity_var:
          ...
          select: "{{macro_datediff('{{user.max_timestamp_bw_tracks_pages}}')}}"
    

Also, if have a case statement, then you can add something like the following:

`select: CASE WHEN {{user.max_timestamp_tracks}}>={{user.max_timestamp_pages}} THEN {{user.max_timestamp_tracks}} ELSE {{user.max_timestamp_pages}} END`
...
The first Profiles project run usually takes longer, especially if you are building predictive features. Training ML models requires generating multiple snapshots of historical data, which significantly increases the initial processing time.

The computational workload varies between different phases:

  * The training phase is computationally intensive as it processes historical data snapshots.
  * The prediction phase is relatively faster once the model is trained.


Your project’s runtime will typically see periodic spikes coinciding with model refreshes, which occur based on the training validity time configured in your propensity model. Between these refreshes, routine runs should complete more quickly.
...
Whereas, in activation API, RudderStack syncs the entire row as value for the `model_name` key:
...
**Can I use`main_id` when searching for a profile in the Profiles Lookup tab?**

Yes. In the **Profile Lookup** tab, you can search using any available unique identifier such as `email`, `phone number`, `user id`, `anonymous id`, or the entity’s `main_id` (for example, `user_main_id`). Enter the value and click **Search user profile** to view the stitched profile.
...
[![](/docs/images/profiles/activation-id.webp)](</docs/images/profiles/activation-id.webp>)
...
Profiles UI supports repos hosted on GitHub, BitBucket, Azure DevOps, and GitLab.

* * *

## Python-related errors

#### How do I resolve the error `rpc init: timeout waiting for python client to initialize` when running the PyNative models (propensity, audit, tutorial)?

PyNative models are written in Python where the code is first compiled into bytecode and then executed by the Python interpreter.

Sometimes, compiling Python code may take more time than expected. You can keep retrying the commands if they do not succeed the first time - the error will resolve on its own.

#### How do I resolve the Python package-related issues when trying out PyNative models?

RudderStack highly recommends using an isolated and clean [Python virtual environment](<https://www.rudderstack.com/docs/archive/profiles/0.20/dev-docs/create-new-project/#create-virtual-environment>).

If the issues persist, you can enforce `pb` to use the Python present in the virtual environment by passing its path as the value for the parameter `--rpc_python_path`.

Make sure to pass the absolute path and not the relative path. For example:
    
    
    pb tutorial --connection <connection_name> --rpc_python_path=/Users/<name>/.env/bin/python3.10
    

> ![info](/docs/images/info.svg)
> 
> You can use the `which python` or `which python3` command to get the Python path.
...
Your user requires privileges to create a symlink. You may either [grant extra privileges to the user](<https://portal.perforce.com/s/article/3472>) or try with a user containing Admin privileges on PowerShell. In case that doesn’t help, try to install and use it via WSL (Widows subsystem for Linux).
...
See [Supported Git URLs](<>) for more information.
...
[![](/docs/images/profiles/profiles-seq-number.webp)](</docs/images/profiles/profiles-seq-number.webp>)
...
**How do I identify the most recently-generated tables for a particular Profiles project?**

Check the `material_registry_4` table in your schema. The `seq_no` column increments with each run - if you are not using the timegrains feature, then for each run there will be only one `seq_no`.

Note that there will be multiple rows in this table corresponding to each model for each `seq_no`.
...
You might get this error due to changes in your Git credentials. Try resetting them in your system configuration and try again.
...
After enabling SSO on GitHub, you must edit your Profiles project and deploy the updated SSH key. Also, make sure you are logged in to GitHub using SSO while doing that.
  ...
  * [![](/docs/images/previous.svg)Previous](</docs/profiles/additional-resources/glossary/>)
  * [Next ![](/docs/images/next.svg)](</docs/profiles/changelog/>)