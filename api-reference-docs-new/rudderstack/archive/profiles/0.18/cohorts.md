        ...
        model_spec:
          ...
          filter_pipeline:
            - type: exclude
              ...
              value: "{{ user.id_type_email_count}} = 0"
            - type: include
    ...
    models:
      -  name: us_credit_card_users
         ...
         model_spec:
           ...
           filter_pipeline:
             - type: include
               value: "{{ knownUsUsers.has_credit_card }} = 1"
             - type: include
               value: "{{ user.last_transaction_value }} > 10000"
...
The following example shows how to define a feature view for the `knownUsUsers` cohort:
    ...
    models:
      - name: knownUsUsers
        ...
        model_spec:
          extends: users/all
          display_name: Known US Users
          materialization:
            output_type: table
          filter_expression:
            AND:
              - "NOT {{ user.id_type_email_count }} = 0" # exclude users which don't have any linked email.
              - "{{ user.country }} = 'US'" # include users with country US.
...
Run your [Profiles CLI project](<https://www.rudderstack.com/docs/archive/profiles/0.18/get-started/profile-builder/>) using the `pb run` command to generate output tables.

### Profiles UI

To view cohorts in the RudderStack dashboard, you can make your Profiles CLI project available in a Git repository and import it in the RudderStack dashboard. See [Import Profiles Project from Git](<https://www.rudderstack.com/docs/archive/profiles/0.18/get-started/import-from-git/>) for more information.
...
You can further activate your cohorts data by syncing it to the downstream destinations. See [Activations](<https://www.rudderstack.com/docs/archive/profiles/0.18/activations/>) for more information.

  * [![](/docs/images/previous.svg)Previous](</docs/archive/profiles/0.18/cli-user-guide/commands/>)
  * [Next ![](/docs/images/next.svg)](</docs/archive/profiles/0.18/activations/>)