...
There are various attributes related to an entity whose profile you are trying to create. For example, they can be `name`, `city`, `LastVisitTimestamp`, etc. for the `user` entity. Each attribute is called an `entity_var`, and it is derived by performing calculation or aggregation on a set of values. Together, all the attributes create a complete picture of the entity. By default, every `entity_var` gets stored as a feature, such as `days_active`, `last_seen`, etc. See [Input Var Vs. Entity Var](<https://www.rudderstack.com/docs/archive/profiles/0.18/additional-concepts/input-entity-var/>) for more information.
...
Once you train a model on historic users, you can use it to make predictions for new users. You need to keep retraining the model as you get new data so that the model continues to learn any emerging patterns in the new users.
  ...
  * [![](/docs/images/previous.svg)Previous](</docs/archive/profiles/0.18/resources/>)
  * [Next ![](/docs/images/next.svg)](</docs/archive/profiles/0.18/resources/yaml-refresher/>)