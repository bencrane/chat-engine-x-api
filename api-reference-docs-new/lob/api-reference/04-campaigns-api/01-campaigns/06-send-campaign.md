...
| `use_type` | string or null | Enum: `"marketing"` `"operational"` `null`. The use type for each mailpiece. Can be one of marketing, operational, or null. Null use_type is only allowed if an account default use_type is selected in Account Settings. |
