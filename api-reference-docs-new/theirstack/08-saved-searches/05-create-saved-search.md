# TheirStack - Saved Searches - Create Saved Search
...
**Endpoint:** `POST /v0/saved_searches`
...
|---|---|---|---|---|
| `name` | `string` (nullable) | No | — | Name of the saved search |
| `body` | `CompanySearchFilters \| JobSearchFilters` | Yes | — | Filters. CompanySearchFilters if type is 'companies', JobSearchFilters if type is 'jobs' |
| `type` | `string` | Yes | — | Type of the saved search. Values: `companies`, `jobs` |
| `is_alert_active` | `boolean` | No | `false` | Is the alert active |
...
```bash
curl -X POST "https://api.theirstack.com/v0/saved_searches" \
