# USAspending API v2 — Endpoint Index
...
| `/api/v2/budget_functions/list_budget_subfunctions/` | POST | Returns all Budget Subfunctions associated with a TAS, ordered by Budget Function code |
...
| `/api/v2/download/status/` | GET | Gets the current status of a download job requested with the `v2/download/awards/` or `v2/download/transaction/` endpoint that same day |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v2/financial_spending/major_object_class/` | GET | Returns financial spending data by major object class for the latest quarter based on the given fiscal year |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v2/subawards/` | POST | Returns subawards optionally related to a specific parent award, or for all parent awards |
