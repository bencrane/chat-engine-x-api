...
> **Danger:** The parameters included in StatusCallback events vary by channel and event type and are subject to change in the future. Twilio will occasionally add new parameters without advance notice. When integrating with webhooks, it is important that your implementation is able to accept and correctly run signature validation on an evolving set of parameters. We strongly recommend using the provided signature validation library from a Twilio SDK and not implementing your own signature validation.
...
Twilio may include the following optional parameters for inbound rich messages. The parameters vary depending on the rich message type:
...
| InteractiveData | The JSON object that contains the omnichannel rich-feature response. See example. |
| FlowData | The serialized payload that WhatsApp returns after the user completes a flow. Twilio passes this payload through in the same stringified format received from WhatsApp. See example. |
| ChannelMetadata | The complete JSON response that Twilio receives from the rich-messaging channel. See example. |
...
The following table describes parameters Twilio includes for incoming messages that share a location:
