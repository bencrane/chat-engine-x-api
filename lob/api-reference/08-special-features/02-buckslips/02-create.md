...
|---|---|---|---|
| `front` | `remote_file_url` (string) or `local_file_path` (string) | **Yes** | A PDF template for the front of the buckslip. |
| `description` | string or null (buckslip_description) <= 255 characters | No | Description of the buckslip. |
| `size` | string | No | Default: `"8.75x3.75"`. Value: `"8.75x3.75"`. The size of the buckslip. |
| `back` | `remote_file_url` (string) or `local_file_path` (string) | No | A PDF template for the back of the buckslip. |
...
| `description` | string or null (buckslip_description) <= 255 characters | **Yes** | Description of the buckslip. |
| `id` | string (buckslip_id) `^bck_[a-zA-Z0-9]+$` | **Yes** | Unique identifier prefixed with `bck_`. |
...
| `url` | string \<uri\> [1..2083] characters | **Yes** | The signed link for the buckslip. |
| `raw_url` | string \<uri\> [1..2083] characters | **Yes** | The raw URL of the buckslip. |
| `front_original_url` | string \<uri\> [1..2083] characters | **Yes** | The original URL of the front template. |
| `back_original_url` | string \<uri\> [1..2083] characters | **Yes** | The original URL of the back template. |
...
| `buckslip_orders` | Array of objects (buckslip_order) >= 0 items | **Yes** | An array of buckslip orders that are associated with the buckslip. |
...
| `deleted` | boolean (deleted) | No | Only returned if the resource has been successfully deleted. |
