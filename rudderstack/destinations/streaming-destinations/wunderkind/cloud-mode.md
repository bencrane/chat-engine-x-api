# Wunderkind Cloud Mode Integration Beta

Send events to Wunderkind using RudderStack cloud mode.
...
After you have successfully instrumented Wunderkind as a destination in RudderStack, follow this guide to correctly send your events to Wunderkind in [cloud mode](<https://www.rudderstack.com/docs/destinations/rudderstack-connection-modes/#cloud-mode>).

Find the open source transformer code for this destination in the [GitHub repository](<https://github.com/rudderlabs/rudder-transformer/tree/main/src/cdk/v2/destinations/wunderkind>).
...
> 
> As Wunderkind is a custom destination, you must send the payload in a format acceptable by Wunderkind Lambda. Reach out to wunderkind team for more information on the Lambda payload schema.
...
The above `track` call is transformed as follows before being sent to Wunderkind:
...
RudderStack does the following before sending events to Wunderkind:

  * Maps `properties` to `attributes` in Wunderkind.
  * Maps `attributes` to `user_attributes` in Wunderkind.
  * Maps the extra properties under `integrations.Wunderkind.extraEventProperties` to the Wunderkind event’s root level properties.
  * Maps the request level properties under `integrations.Wunderkind.lambdaRootLevelProperties` to Wunderkind request’s root level properties.