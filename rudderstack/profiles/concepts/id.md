...
Creating a Customer360 (C360) is primarily combining data from different sources, or [`inputs`](<https://www.rudderstack.com/docs/profiles/concepts/inputs/>). ID fields define how those tables are connected.

You can create the [`features`](<https://www.rudderstack.com/docs/profiles/concepts/features/>) of a C360 by chaining these IDs from different inputs together. Note that IDs are not always consistent between `inputs` in name or format.

Profiles has two concepts related to IDs:

  * `id`: This is the field within `inputs` that is an identifier for a given [entity](<https://www.rudderstack.com/docs/profiles/concepts/entities/>). For example a user name, an email, or an anonymous ID.
...
You can define `id_types` in the [`pb_project.yaml`](<https://www.rudderstack.com/docs/profiles/dev-docs/pb-project-yaml/>) file present in the top level of every Profiles project, as shown:
...
You can then define `ids` with the ID types in the [`inputs.yaml`](<https://www.rudderstack.com/docs/profiles/dev-docs/inputs-yaml/>) file. Each `inputs` ID is labeled with one of the `id_type` defined in [`pb_project.yaml`](<https://www.rudderstack.com/docs/profiles/dev-docs/pb-project-yaml/>).
...
## Best practices
  ...
  * You can define `id_types` for the [`id_graph`](<https://www.rudderstack.com/docs/profiles/concepts/identity-graph/>) or to connect additional tables for `features`.
  ...
  * [![](/docs/images/previous.svg)Previous](</docs/profiles/concepts/entities/>)
  * [Next ![](/docs/images/next.svg)](</docs/profiles/concepts/identity-graph/>)